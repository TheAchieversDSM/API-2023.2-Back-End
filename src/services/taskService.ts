...
class TaskService {
  ...
  constructor() {
    ...
    this.mongoFutureTaskRepository =
      MongoDataSource.getMongoRepository(MongoFutureTask);
    this.mongoDeleteHistoricoRepository =
      MongoDataSource.getMongoRepository(DeleteHistoricoTask);
  }
  ...
  public async createTask(task: Task) {
    try {
      ...
      for (const userId of sharedUserIds) {
        await DataBaseSource.getRepository("user_task").insert({
          userId: userId,
          taskId: newTask.id,
        });
      }
  ...
  public async shareTask(taskId: number, usersIds: number[]) {
    try {
      ...
      for (const userId of usersIds) {
        try {
          const result = await DataBaseSource.getRepository("user_task").insert(
          ...
          if (error.code === "ER_DUP_ENTRY") {
            console.log(
              `Duplicate entry: UserId: ${userId} and taskId: ${taskId}`
            );
          } else {
            console.log(
              `Error to insert: UserId: ${userId} and taskId: ${taskId}`,
              error
            );
          }
        }
  ...
  public async stopTaskSharing(taskId: number, usersIds: number[]) {
    try {
      for (const userId of usersIds) {
        await DataBaseSource.getRepository("user_task").delete({
          taskId: taskId,
          userId: userId,
        });
      }
  ...
  public async getExpiredTasks(
    userId: number,
    date: string
  ): Promise<{ recorrente: any[]; naoRecorrente: any[]; error?: unknown }> {
    try {
      ...
      tasks = await Promise.all(
        tasks.map(async (task: any) => {
          let tsk = await this.taskRepository
            .createQueryBuilder("task")
            .innerJoinAndSelect("task.users", "users")
            .where("task.id = :id", { id: task.id })
            .getOne();

          let userOwner = (await userService.getUserById(
            task?.userId as number
          )) as User;
          if (userOwner) {
            task.userOwner = userOwner;
          }
          if (tsk) {
            task.users = tsk.users as User[];
          }
      ...
      const naoRecorente = tasks.filter(
        (task: Task) => task.customInterval === 0
      );

      const pastCycleTasks = await this.mongoTaskRepository.find({
  ...
  public async getTimeSpentMonthly(userId: number, year: number) {
    try {
      const tasks: Task[] = await this.taskRepository
        ...
        .where("task.userId = :userId", { userId })
        .andWhere(
          'DATE_FORMAT(task.createdAt, "%Y-%m-%d") BETWEEN :start AND :end',
          {
            start: `${year}-01-01`,
            end: `${year}-12-31`,
          }
  ...
  public async getAllNonCyclicTasks(userId: number) {
    try {
      ...
      console.log(tasks);
      const nonCyclicTasks = tasks.filter((task) => {
        return task.customInterval === 0;
      });
  ...
  public async getAllCyclicTasks(userId: number) {
    try {
      ...
      const cyclicTasks = tasks.filter((task) => {
        return task.customInterval > 0;
      });
      return cyclicTasks;
    } catch (error) {
  ...
  public async checkTaskToRenew(cyclicTaks: Task[]) {
    try {
      ...
      const tasksToRenew = cyclicTaks.filter((task) => {
        return (
          moment(task.lastExecution)
            .add(task.customInterval, "days")
  ...
  public async deleteTask(id: number, userId: number) {
    const mongoFutureTaskRepository =
      MongoDataSource.getMongoRepository(MongoFutureTask);
    ...
    try {
      ...
      if (task.userId !== userId) {
        throw new Error(
          "Permission denied. You do not have permission to delete this task."
        );
      }
  ...
  public async refreshTask(taskId: number) {
    try {
      ...
      let subtasks = (await subtaskService.getSubtasksByTask(
        taskId
      )) as Subtask[];
  ...
  public async deleteFutureTask(taskId: number) {
    try {
      ...
      const deletedTask = await this.mongoFutureTaskRepository.delete({
        taskId: taskId,
        deadline: today.format("YYYY-MM-DD"),
      });
      if (!deletedTask.affected) {
        throw new Error("Task not found");
      }
  ...
  public async cloneTask(taskId: number) {
    try {
      ...
      task.subtask = (await subtaskService.getSubtasksByTask(
        taskId
      )) as Subtask[];
  ...
  public async createFutureTasks(task: Task) {
    try {
      ...
      taskInfo.subtask = (await subtaskService.getSubtasksByTask(
        taskInfo.id as number
      )) as Subtask[];
      ...
      for (let index = 0; index < 30; index++) {
        ...
        createdDate
          .add(taskInfo.customInterval, "days")
          .tz("America/Sao_Paulo")
          .format("YYYY-MM-DD");
  ...
  public async HistoricDeleteTask(id: number, userId: number, message: string) {
    try {
      ...
      let user = await DataBaseSource.getRepository(User).findOne({
        where: { id: userId },
      });
      ...
      const historic = await this.mongoDeleteHistoricoRepository.save(
        DeleteHistoricoTask
      );
      return historic;
    } catch (error: any) {
  ...
  public async getHistoricDeleteTask(
    idTask: number
  ): Promise<DeleteHistoricoTask[]> {
    try {
      const findTasks = await this.mongoDeleteHistoricoRepository.find({
        where: { taskId: { $eq: idTask } },
      });
      return findTasks;
    } catch (error: any) {
      throw new Error(error);
  ...
  public async getHistoricDeleteTaskByUser(
    idUser: number
  ): Promise<DeleteHistoricoTask[]> {
    try {
      const findTasks = await this.mongoDeleteHistoricoRepository.find({
